...
vim.api.nvim_create_user_command("YaziTmux", function()
  ...
  local open_callback = vim.schedule_wrap(function()
    ...
    if #files > 1 then
      vim.notify("Got more than one file from Yazi - opening the first one and ignoring the rest.", vim.log.levels.WARN)
    end
